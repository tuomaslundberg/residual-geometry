"""Data loading, toy data generation, and basic corpus utilities."""
...
def load_jsonl(path: str | Path) -> list[Document]:
    ...
    Each line must have keys: ``text``, ``language``, ``register``.
    """
    docs: list[Document] = []
    with open(path) as f:
        for line in f:
...
def make_toy_data(
    ...
    """Generate synthetic toy documents (no real text — pipeline smoke test only)."""
    rng = np.random.default_rng(seed)
    docs: list[Document] = []
    for lang in languages:
        for reg in registers:
            for i in range(n_per_class):
                # Placeholder text that encodes class identity for smoke tests.
